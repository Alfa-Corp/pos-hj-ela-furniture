<template>
    <Head>
        <title>Riwayat Produk Keluar - Sistem Kasir</title>
    </Head>
    <main class="c-main">
                    ...
                    <div class="col-md-12">
                        <div class="card border-0 rounded-3 shadow border-top-purple">
                            <div class="card-header">
                                <span class="font-weight-bold"><i class="fa fa-shopping-bag"></i> HISTORY OUT PRODUCT {{ title ? title : "" }}</span>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="export text-end mb-3">
                                    <a :href="`/apps/products/${product_id}/out/export?start_date=${start_date}&end_date=${end_date}`" target="_blank" class="btn btn-success btn-md border-0 shadow me-3"><i class="fa fa-file-excel"></i> EXCEL</a>
                                    <a :href="`/apps/products/${product_id}/out/pdf?start_date=${start_date}&end_date=${end_date}`" target="_blank" class="btn btn-secondary btn-md border-0 shadow"><i class="fa fa-file-pdf"></i> PDF</a>
                                </div>
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <tr v-for="(product, index) in products.reseller_transaction_details" :key="index">
                                    ...
                                    <tbody>
                                        <tr v-for="(product, index) in products.umum_transaction_details" :key="index">
                                            <td>{{ product.created_at }}</td>
                                            <td>DIRECT</td>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            const filter = () => {
                Inertia.get(`/apps/products/${props.products.id}/out/filter`, {
                    // send data to server
