<template>
    <Head>
        <title>Transaksi Reseller - Sistem Kasir</title>
    </Head>
    <main class="c-main">
                        ...
                        <div class="card border-0 rounded-3 shadow">
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" :value="product.title" placeholder="Nama Produk" readonly>
                                </div>
                                <div class="mb-3">
                        ...
                        <div class="card border-0 rounded-3 shadow">
                            <div class="card-body">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr v-for="cart in carts" :key="cart.id">
                                            ...
                                            <td>Rp. {{ formatPrice(cart.product.sell_price_reseller) }}</td>
                                ...
                                <div class="text-end mt-4">
                                    ...
                                    <button v-if="hasAnyPermission(['reseller_transactions.order'])" @click.prevent="storeTransaction" class="btn btn-purple btn-md border-0 shadow text-uppercase m-2" :disabled="cash < grandTotal || grandTotal <= 0 ">Pay Order & Print</button>
                                    <button v-if="hasAnyPermission(['reseller_transactions.retur'])" @click.prevent="storeTransaction" class="btn btn-danger btn-md border-0 shadow text-uppercase" :disabled="cash > grandTotal || grandTotal >= 0 ">Pay Retur & Print</button>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        setup(props) {
            ...
            const addToCart = () => {
                ...
                //send data to server
                Inertia.post('/apps/reseller_transactions/addToCart', {

                    //data
                    product_id: product.value.id,
                    qty: qty.value,
                    sell_price: product.value.sell_price_reseller,
            ...
            const destroyCart = (cart_id) => {
                Inertia.post('/apps/reseller_transactions/destroyCart', {
                    cart_id: cart_id
                }, {
                    onSuccess: () => {
            ...
            const storeTransaction = () => {
                // HTTP request
                axios.post('/apps/reseller_transactions/store', {
                    //send data to server
                    ...
                    Swal.fire({
                        ...
                        setTimeout(() => {
                            ...
                            //print
                            window.open(`/apps/reseller_transactions/print?invoice=${response.data.data.invoice}`, '_blank');
